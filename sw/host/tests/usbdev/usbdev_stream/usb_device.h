...
class USBDevice {
 public:
  USBDevice(bool verbose = false)
      : verbose_(verbose), manual_(false), state_(StateStreaming), ctx_(nullptr), devh_(nullptr) {}
...
//    StateActive,
  ...
  // Maximum packet size of 64 bytes is used for Control, Interrupt and Bulk Transfers.
   ...
   */
  void FillControlTransfer(struct libusb_transfer *xfr, uint8_t ep, uint8_t *buffer, libusb_transfer_cb_fn callback, void *user_data, unsigned timeout_ms) {
    libusb_fill_control_transfer(xfr, devh_, buffer, callback, user_data, timeout_ms);
    // Complete the endpoint number because we're _not_ always targeting Endpoint Zero
    xfr->endpoint = ep;
  }
  /**
   ...
   */
  uint8_t TestArg(unsigned arg) const { return (arg < 4U) ? testArg_[arg] : 0U; }
  /**
