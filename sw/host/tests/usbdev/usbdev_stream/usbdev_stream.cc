...
bool USBDevStream::ProvisionSpace(uint8_t **space, uint32_t len, bool contig) {
  uint32_t space_bytes = SpaceAvailable(space, contig);
  if (space_bytes >= len) {
    return true;
  }
  ...
  // buffer, wrap sooner.
  if (contig && buf_.rd_idx <= buf_.wr_idx &&
      (kBufferSize - buf_.wr_idx) < len && buf_.rd_idx > len) {
...
uint32_t USBDevStream::DataAvailable(uint8_t **data, bool contig) {
  ...
  if (buf_.rd_idx >= buf_.end_idx &&
      buf_.wr_idx > 0U && buf_.wr_idx < buf_.rd_idx) {
    buf_.rd_idx = 0U;
  }
...
bool USBDevStream::ConsumeData(uint32_t len) {
  ...
  uint32_t data_bytes = DataAvailable(nullptr, true);
  if (data_bytes < len) {
    return false;
  }
  ...
  // TODO: this can be simplified if we drop contig option
