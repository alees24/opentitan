...
void USBDevIso::Close(bool wait) {
  SetClosing(true);

// TODO: migrate this to suspend/disconnect/sleep?!
  if (wait) {
...
bool USBDevIso::Completed() const {
  ...
  // side software ultimately decides success or failure of the test.
  return false;
}

// Return a summary report of the stream settings of status.
std::string USBDevIso::Report(bool status, bool verbose) const {
  return "";
}
