...
static int RunTest(USBDevice *dev, unsigned nstreams, uint32_t transfer_bytes,
  ...
  for (unsigned idx = 0U; idx < nstreams; idx++) {
    ...
    switch (testNum) {
      ...
      case USBDevice::kUsbTestNumberMixed: {
        uint32_t mixedTypes = (testArg[3] << 16) | (testArg[2] << 8) | testArg[1];
        ...
        switch ((mixedTypes >> (idx * 2)) & 3U) {
          case 0U: streamType = USBDevStream::StreamType_Control; break;
          case 1U: streamType = USBDevStream::StreamType_Isochronous; break;
          case 2U: streamType = USBDevStream::StreamType_Bulk; break;
          default: streamType = USBDevStream::StreamType_Interrupt; break;
        }
      } break;
    ...
    std::cout << "S" << idx << ": " << USBDevStream::StreamTypeName(streamType) << std::endl;
    ...
    switch (streamType) {
      ...
      case USBDevStream::StreamType_Control: {
        USBDevCtl *ctl = new USBDevCtl(dev, idx, transfer_bytes, cfg.retrieve, cfg.check, cfg.send, cfg.verbose);
        if (ctl) {
          opened = ctl->Open(idx);
          if (opened) {
            streams[idx] = ctl;
  ...
  // TODO: Resume Signaling shall occur for at least 20ms; we don't control this
  // there's little point us trying to communicate sooner.
  ...
  do {
    ...
    if (std::abs((int32_t)total_sent - (int32_t)prev_bytes) >= 0x1000 || done) {
      ...
      // side.
      //
      // TODO: perhaps introduce some way for the device to signal the test
      // completion status to the host, via a reliable channel! we could
      // perhaps read the Test Status via a Vendor-Specific Control Transfer

      uint32_t bytes_left = (total_sent < total_bytes) ? (total_bytes - total_sent) : 0U;

      char transBytes[11 * 9];
      char bufBytes[11 * 9];
      char *tp = transBytes;
      char *bp = bufBytes;
      for (unsigned idx = 0U; idx < nstreams; idx++) {
        tp += sprintf(tp, "%08x ", streams[idx]->TransferBytes());
        bp += sprintf(bp, "%08x ", streams[idx]->BytesSent());
      }
      *tp = *bp = '\0';
//      std::cout << transBytes << std::endl;
//      std::cout << bufBytes << std::endl;
      std::cout << "Bytes left: 0x" << std::hex << bytes_left << "         \r" << std::dec << std::flush;
...
int main(int argc, char *argv[]) {
  ...
  // TODO: Ascertain the port names by inquiring of libusb or other sw layer.
  if (!out_port) {
  ...
  int rc = RunTest(&dev, nstreams, transfer_bytes, in_port, out_port);
