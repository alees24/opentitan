...
void USBDevSerial::Close(bool wait) {
...
// TODO: tidy this by having one function to open ports from names
// TODO: I don't think we can expect this to suspend/resume at the byte level
...
bool USBDevSerial::Resume() {
  const char *out_name = outPort_.c_str();
  const char *in_name = inPort_.c_str();
...
std::string USBDevSerial::Report(bool status, bool verbose) const {
  return "";
}
