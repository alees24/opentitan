...
//
// IRQ-driven USB device test
//
// This is a basic test of interrupt-driven operation of the usb_testutils layer
// and usbdev, mimicking the behavior of usbdev_test.c
...
bool test_main(void) {
  LOG_INFO("Running USBDEV_IRQ test");
  ...
  ibex_timeout_t next = ibex_timeout_init(1000000u);
  unsigned nsecs = 0u;
  while (usbdev_control.device_state != kUsbTestutilsDeviceConfigured) {
    if (ibex_timeout_check(&next)) {
      LOG_INFO("%u second(s)", nsecs);
      next = ibex_timeout_init(1000000u);
      nsecs++;
    }
  }
  ...
  while (usb_chars_recved_total < kExpectedUsbCharsRecved) {
    // TODO: we should perhaps sleep here for a while, but this provides at
    // least a simple progress indicator whilst the USB device configuration
    // and traffic occurs in the background
    if (ibex_timeout_check(&next)) {
      LOG_INFO("%u second(s)", nsecs);
      next = ibex_timeout_init(1000000u);
      nsecs++;
