...
class usbdev_device_timeout_vseq extends usbdev_base_vseq;
  ...
  virtual task body();
    ...
    // TODO: to avoid being too sensitive here, we could perhaps provide an exclusion window
    // around the critical timeout decision, because of CDC rand delays, internal details of DUT
    // etc. This is a USB timing characteristic we're supposed to be checking, not a DUT property.
    ...
    for (int unsigned txn = 0; txn < num_trans; txn++) begin
      ...
      // TODO: decide upon and tidy this!
      ...
      case (response)
        ResponseSETUP, ResponseIN, ResponseOUT: begin
          ...
          // TODO: decide upon and tidy this!
          do_not_wait = (response == ResponseIN);
          send_token_packet(ep_default, pid);
          ...
          // TODO: Proposed RTL change (24/06/28) deasserts 'sending' in the event of rollback.
          exp_sending = 1'b0;
          delay = 1;
        end
        ResponseAck: begin
