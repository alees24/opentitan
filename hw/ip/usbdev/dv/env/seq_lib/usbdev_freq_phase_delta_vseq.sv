...
class usbdev_freq_phase_delta_vseq extends usbdev_stream_len_max_vseq;
  ...
  // Min/max DUT clock frequencies according to the USB 2.0 specification; Table 7-9
  ...
  constraint host_target_freq_khz_c {
    // Maximum frequency extremes (scaled from 12Mbps to 48MHz clock). See USB 2.0 Table 7-9
    host_target_freq_khz >= 47_880 && host_target_freq_khz <= 48_120;
  };
  ...
  // Are we require to track the host frequency by adjusting the DUT oscillator?
  bit osc_tracking = 0;
  ...
  // Indicate that the test sequence is terminating.
  bit stopping = 0;
  ...
  virtual task host_drift();
    ...
    while (!stopping && elapsed_cycles < total_cycles) begin
  ...
  virtual task bus_traffic();
    int unsigned frames = num_frames;
    while (frames > 0) begin
    ...
    // Signal to the other processes that the sequence is stopping.
    stopping = 1;
  endtask

  virtual task pre_start();
    // Vary the initial DUT clock frequency in these test sequences.
    cfg.usb_clk_freq_khz_c.constraint_mode(0);
    if ($value$plusargs("usb_freq_delta=%0d", usb_freq_delta)) begin
      ...
      // The DUT frequency is kept constant by default in other vseqs, but we want to vary it for
      // freq/phase testing.
    ...
    if ($value$plusargs("host_freq_delta=%0d", host_freq_delta)) begin
      cfg.host_clk_freq_khz_c.constraint_mode(0);
