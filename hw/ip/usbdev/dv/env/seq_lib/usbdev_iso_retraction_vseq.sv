...
class usbdev_iso_retraction_vseq extends usbdev_base_vseq;
  ...
  // Use isochronous endpoint/transfers?
  // TODO: presently we are concerned only with Isochronous traffic.
  bit iso = 1;
    ...
    do begin
      ...
      case (reply.m_pid_type)
        PidTypeData0, PidTypeData1: begin
          ...
          data_pkt data;
          $cast(data, reply);
          // Isochronous endpoints without pending data shall return a Zero Length Packet.
  ...
  task device_side();
    ...
    repeat (num_trans) begin
      ...
      // TODO: investigate whether there's any benefit to varying this.
    ...
    if (get_field_val(ral.configin[0].rdy, configin)) begin
      retract_in_packet(ep_default, configin, retracted, retracted_buf);
  ...
  virtual task body();
    ...
    // Isochronous traffic?
    // TODO: presently we are concerned only with Isochronous traffic.
    if (iso) csr_wr(.ptr(ral.in_iso[0].iso[ep_default]), .value(1'b1));
...
`uvm_info(`gfn, $sformatf("seq %d : sent %d rcvd %d", seq, pkts_sent[seq], pkts_rcvd[seq]), UVM_MEDIUM)
