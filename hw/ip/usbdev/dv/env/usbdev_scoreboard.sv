...
class usbdev_scoreboard extends cip_base_scoreboard #(
  ...
  function void process_tl_addr(ref tl_seq_item item, input string ral_name);
    ...
    case (1)
      ...
      (!uvm_re_match("intr_enable", csr_name)): begin
        bfm.intr_enable = wdata & IntrMask;
      end
      ...
      (!uvm_re_match("out_data_toggle", csr_name)): begin
        bfm.write_out_toggles(get_field_val(ral.out_data_toggle.mask,   wdata),
                              get_field_val(ral.out_data_toggle.status, wdata));
      end
      (!uvm_re_match("in_data_toggle", csr_name)): begin
        bfm.write_in_toggles(get_field_val(ral.in_data_toggle.mask,   wdata),
      ...
      (!uvm_re_match("phy_pins_sense", csr_name)),
      ...
      (!uvm_re_match("wake_control", csr_name)): begin
  ...
  function void process_tl_data(ref tl_seq_item item, input string ral_name);
    ...
    case (1)
      ...
      (!uvm_re_match("rxenable_out", csr_name)): begin
      ...
      (!uvm_re_match("in_data_toggle", csr_name)): begin
         void'(csr.predict(.value(bfm.in_toggles), .kind(UVM_PREDICT_READ)));
      end
      ...
      (!uvm_re_match("count_out", csr_name)),
      (!uvm_re_match("count_in", csr_name)),
      (!uvm_re_match("count_nodata_in", csr_name)),
      (!uvm_re_match("count_errors", csr_name)): do_read_check = 1'b0;

      // Write Only registers that cannot be altered by the DUT hardware.
      (!uvm_re_match("intr_test", csr_name)),
      (!uvm_re_match("intr_enable", csr_name)),
      (!uvm_re_match("alert_test", csr_name)),
      (!uvm_re_match("wake_control", csr_name)),
      (!uvm_re_match("phy_config", csr_name)),
      (!uvm_re_match("phy_pins_drive", csr_name)),
      (!uvm_re_match("fifo_ctrl", csr_name)): begin
